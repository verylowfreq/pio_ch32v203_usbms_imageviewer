                      ...
                      uint16_t hwColor) //specify the color of the line
{
	...
	if (hwXpos0 == hwXpos1) {
		lcd_draw_v_line(hwXpos0, hwYpos0, hwYpos1 - hwYpos0, hwColor);
	}
	if (hwYpos0 == hwYpos1){
		lcd_draw_h_line(hwXpos0, hwYpos0, hwXpos1 - hwXpos0, hwColor);
	}
...
#endif
	uint16_t endColumn = hwXpos + hwWidth;
	if (endColumn >= LCD_WIDTH) {
		endColumn = LCD_WIDTH - 1;
	}
	uint16_t endRow = hwYpos + hwHeight;
	if (endRow >= LCD_HEIGHT) {
		endRow = LCD_HEIGHT - 1;
	}
	LCD_SetWindow(hwXpos, hwYpos, endColumn, endRow);
	uint32_t pixelcount = (endColumn - hwXpos) * (endRow - hwYpos);
...
#endif
	uint16_t y1 = min(hwYpos + hwHeight, LCD_HEIGHT - 1);

	if (hwXpos >= LCD_WIDTH || hwYpos >= LCD_HEIGHT) {
		return;
	}
	lcd_fill_rect(hwXpos, hwYpos, 1, y1, hwColor);
}
...
#endif
	uint16_t x1 = min(hwXpos + hwWidth, LCD_WIDTH - 1);

	if (hwXpos >= LCD_WIDTH || hwYpos >= LCD_HEIGHT) {
		return;
	}
	
    lcd_fill_rect(hwXpos, hwYpos, x1, 1, hwColor);
}
